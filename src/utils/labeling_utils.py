from src.base.localizator import get_candidates, get_predicted
...
def run_automatic_labeling(subject: str, codename: str, prev_raw = None):
    mode = 'auto'
    eeg_channel, path_file, scoring_path, annotations_path, cut_off_freqs = load_configuration_parameters(subject, codename, mode)
    ...
    if not prev_raw:   
        ...
        # Get predictions
        eeg_signal = raw_with_sleep_stages.get_data(picks=[eeg_channel])[0]
        KC_predicted_onset_duration = get_predicted(eeg_signal, sfreq=raw.info['sfreq'], path_scoring=scoring_path, window_length=1)
        raw_with_candidates = set_candidates_labels(raw_with_sleep_stages, KC_predicted_onset_duration)
    ...
    #Save changes
    if not check_file_if_ready_to_save(raw_new, raw_plot, annotations_path, mode, codename, subject):
        run_automatic_labeling(subject, codename, raw_new)